...
async function runTest() {
    ...
    try {
        ...
        // Scroll until Weekly Usage is visible
        await driver.$(
            'android=new UiScrollable(new UiSelector().scrollable(true)).scrollIntoView(new UiSelector().text("Usage Weekly Usage Keep your streak going In progress"))'
        );
        ...
        const weeklyQuest = await driver.$(
            'android=new UiSelector().text("Usage Weekly Usage Keep your streak going In progress")'
        );

        await weeklyQuest.waitForDisplayed({ timeout: 20000 });
