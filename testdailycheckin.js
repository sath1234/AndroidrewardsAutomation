...
async function runTest() {
    ...
    try {
        ...
        const dailyCheckinQuest = await driver.$(
            '//android.widget.Button[@text="Usage Daily Check-in Earn 20 SP + $0.01 for your first 50 days Checked in today"]'
        );

        await dailyCheckinQuest.waitForDisplayed({ timeout: 20000 });

        try {
            await dailyCheckinQuest.click();
        } catch (e) {
            await driver.execute("mobile: clickGesture", {
                elementId: dailyCheckinQuest.elementId
            });
        }

        console.log("Daily Check-in quest clicked");

        await driver.pause(5000);

        // Close Button (Accessibility ID)
        ...
        try {
            ...
            const response = await axios.get(
                'https://api.santabrowser.com/quests/bff/v1/quests/q.daily.checkin/checkin-status?clid=dd681d267a19a23'
            );

            console.log("\n========== DAILY CHECK-IN API RESPONSE ==========");
